...
export default function RecoverPage() {
  ...
  return <IonPage><IonHeader><IonToolbar><IonButtons slot="start"><IonBackButton defaultHref="/my-events" text="Voltar" /></IonButtons><IonTitle>{status === 'transfer' ? 'Usar meus Boras em outro dispositivo' : 'Recuperar meus Boras'}</IonTitle></IonToolbar></IonHeader><IonContent className="ion-padding form-page"><main className="recovery-container"><IonCard className="recovery-card"><IonCardContent>
    ...
    {status === 'transfer' && <><p className="recovery-lead">Crie um link para acessar seus Boras em outro aparelho. Este aparelho continuará com acesso normalmente.</p><IonButton expand="block" onClick={() => void createLink()} disabled={creating}>{creating ? 'Criando link…' : 'Criar link de acesso'}</IonButton><details className="recovery-options"><summary>Opções avançadas</summary><IonItem button detail={false} lines="none" className="recovery-admin-option" onClick={() => setIncludeAdminAccess((current) => !current)}><IonCheckbox slot="start" checked={includeAdminAccess} onClick={(event) => event.stopPropagation()} onIonChange={(event) => setIncludeAdminAccess(event.detail.checked)} /><IonLabel>Incluir controles de organizador<small>Permite editar, encerrar e excluir os eventos que você criou.</small></IonLabel></IonItem>{includeAdminAccess && <p className="recovery-security-warning"><IonIcon icon={shieldCheckmarkOutline} aria-hidden="true" />Quem receber este link poderá editar e excluir os eventos que você organizou.</p>}</details>{link && <section className="recovery-created-link"><p>Abra a câmera do outro aparelho para escanear o código, ou compartilhe o link.</p>{qrCode && <img className="recovery-qr-code" src={qrCode} alt="QR code para abrir seus Boras em outro dispositivo" />}<div className="recovery-link-actions"><IonButton onClick={() => void shareLink()}>Compartilhar</IonButton><IonButton fill="outline" onClick={() => void copyLink()}>Copiar link</IonButton></div></section>}</>}
  </IonCardContent></IonCard></main></IonContent></IonPage>;
}
